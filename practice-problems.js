//তোমাকে একটা function দেওয়া হবে called mindGame(” যা ইনপুট হিসেবে একটা positive number নিবে।")
//এখন তোমার task : তোমাকে সেই নাম্বার এর সাথে 3 গুন করে, তারপর 10 যোগ করে, তারপর 2 দিয়ে ভাগ করে, তারপর 5 বিয়োগ করে, যা আউটপুট আসবে তা return করতে হবে

// Input & Output

/* Input: 5
Output: 7.5

Input: 50
Output: 75

Input: 33
Output: 49.5 */

// Solution - 1
/*
The function 'mindGame' will return a calculated result of input value. The function accepts one parameter. Input parameter should be a positive number.
*/
function mindGame(number) {
    if( typeof number !== 'number' || number < 0 ) {
        return 'Input value should be a positive number';
    }

    let firstResult =  number * 3;
    let secondResult = firstResult + 10;
    let thirdResult = secondResult / 2;
    let finalResult = thirdResult - 5;

    return finalResult;
}









//Problem 2: Finding even or odd
//তোমাকে একটা function দেওয়া হবে called evenOdd(). এটা ইনপুট হিসেবে একটা string নিবে। 
//এখন তোমার task: String এ total কয়টা character আছে তার উপর ভিত্তি করে আউটপুট দিতে হবে। আউটপুট হবে ‘even’ অথবা ‘odd’ ।Sample Input & Output:
/* 
Input: ‘Phero’
Output: odd

Input: ‘Batch7’
Output: even

Input: ‘chatgpt’
Output: odd */



// Solution - 2
/*
The function will return 'even' when input value length number is an even number
or return 'odd' when input value length number is an odd number. The function accepts 1 parameter. Input parameter should be a string.
*/
function evenOdd(string) {
    if(typeof string !== 'string' ) {
        return 'Input parameter should be a string';
    }

    let stringCount = string.length;
    if( stringCount % 2 == 0 ) {
        return 'even';
    } else {
        return 'odd';
    }
}








// Problem 3: Is Less or Greater than seven


// তোমাকে একটা function দেওয়া হবে called “isLGSeven()”. এটা ইনপুট হিসেবে একটা number নিবে।  
//এখন তোমার task: তোমাকে ইনপুট ভ্যালু এবং ৭ এর মধ্যে পার্থক্য বের করতে হবে। যদি এই পার্থক্য ৭ এর চেয়ে ছোট হয়, তখন তোমাকে return করতে হবে সেই বিয়োগফল। নাহলে তোমাকে return করতে হবে double of the input। মানে যে সংখ্যা ইনপুট হিসেবে নিয়েছো সেটির দ্বিগুণ।

/* Input : 6
Output: -1

Input: -15
Output: -22

Input: 15
Output: 30
 */


// Solution - 3
/*
The function will return a number value.
Firstly get a difference between input parameter and 7. If the difference is less then 7, than it will return the difference nor it will return double of input value. The function accepts 1 parameter. Input parameter should be a number.
*/
function isLGSeven(input) {
    if( typeof input !== 'number' ) {
        return 'Input parameter should be a number';
    }
    let difference = input - 7;
    if( difference < 7 ) {
        return difference;
    } else {
        let doubleOfInput = input * 2;
        return doubleOfInput;
    }
}





// Problem 4: Finding Bad data

// তোমাকে একটা function দেওয়া হবে called findingBadData(). এটা ইনপুট হিসেবে একটা array নিবে। array তে যেকোনো সংখ্যক কতগুলো number থাকবে। number গুলো negative number (less than zero) ও হতে পারে, আবার positive number (greater than or equal to zero) ও হতে পারে। কোনও number যদি negative হয় সেটাকে আমরা বলব “Bad Data”. কোনও number যদি positive হয় সেটাকে আমরা বলব “Good Data”. 

//এখন তোমার task: array তে কতগুলো Bad data আছে সেটা খুঁজে বের করতে হবে এবং সেই নাম্বার টা return করতে হবে।
/* 
Input: [ 1,2,5 ]
Output: 0

Input: [ 2, -5, -7, -13 ]
Output: 3

Input: [ -4, -9, -5, -33, -55 ]
Output: 5
 */


// Solution - 4
/*
The function will return counted number of negetive values from an array. The Function accepts one parameter. Input value should be an array.
*/
function findingBadData(ages) {
    if( Array.isArray(ages) == false ) {
        return 'Input parameter should be an array';
    }

    let badData = [];
    for( let i = 0; i < ages.length; i++ ) {
        let age = ages[i];
        if( age < 0 ) {
            badData.push(age);
        }
    }
    return badData.length;
}


// Problem 5: Convert your gems into diamond
// তোমাকে একটা function দেওয়া হবে called gemsToDiamond(). এটা ইনপুট হিসেবে তিনটা number ( তোমাদের ৩ বন্ধুর gems এর সংখ্যা ) নিবে। 
/* 
১ম বন্ধুর প্রতি gems এর ক্ষমতা = 21
২য় বন্ধুর প্রতি gems এর ক্ষমতা = 32
৩য় বন্ধুর প্রতি gems এর ক্ষমতা = 43
 */
// gems এর ক্ষমতা বলতে একটা gems কে কয়টা diamond এ convert করা যাবে সেটা বুঝানো হচ্ছে। ]

// এখন তোমার task: সব বন্ধুর gems মিলিয়ে total কতটি diamond পাবে সেটা বের করতে হবে। total diamond এর সংখ্যা যদি 1000 এর দিগুন এর উপর হয়ে যায় সেক্ষেত্রে total diamond থেকে 2000 বাদ দিয়ে যতগুলো diamond অবশিষ্ট থাকবে, ততগুলো তোমরা পাবে।
/* 
Input: 1, 1, 1
Output: 96

Input: 20, 200, 50
Output: 6970

Input: 100, 5, 1
Output: 303
 */


// Solution - 5
/*
The function will return three friends rest diamond. If their total diamond is more than double of 1000 than rest diamon except 2000 will be returned. if total is not more then double of 1000 than returns total diamonds. The function accepts three parameters. Parameters should be number.
*/
function gemsToDiamond( friend1Gem, friend2Gem, friend3Gem ) {
    if( typeof friend1Gem !== 'number' || typeof friend2Gem !== 'number' || typeof friend3Gem !== 'number' ) {
        return 'Input parameters should be number and pass 3 parameters';
    }

    const friend1GemPower = 21;
    const friend2GemPower = 32;
    const friend3GemPower = 43;

    let friend1Diamond = friend1Gem * friend1GemPower;
    let friend2Diamond = friend2Gem * friend2GemPower;
    let friend3Diamond = friend3Gem * friend3GemPower;

    let totalDiamond = friend1Diamond + friend2Diamond + friend3Diamond;
    
    if( totalDiamond > 1000*2 ) {
        let restDiamond = totalDiamond - 2000;
        return restDiamond;
    } else {
        return totalDiamond;
    }
}
