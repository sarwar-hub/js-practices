...
const student = {
    name: 'Sarwar',
    roll: 12,
    subjects: ['Organic', 'Physical', 'Analytical'],
    marks: {organic: 60, physical: 50, analytical: 70},
    totalMark: function() {
        return this.marks.organic + this.marks.physical + this.marks.analytical;
    }
}

const total = student.totalMark()
//console.log(total);
...
const comment = `${student.name} got ${student.marks.organic} in ${student.subjects[0]} and total mark is ${student.totalMark()}`;
...
const result2 = number => number / 17;


const returnValue2 = result2(34);
...
const result3 = (number1, number2) => (number1 + number2) / 2;

const returnValue3 = result3(5,5);
...
const returnValue4 = result4(7,7);
...
const numbers = [14,21,35,42,56];
const calculated = numbers.map(x => x/7);
...
const account = {name: 'sarwar', accno: 22273, balance: 89};
const {accno} = account;
//console.log(accno);


const account2 = ['sarwar', 89, 22273];
const [name, balance, ...rest] = account2;
//console.log(balance);
